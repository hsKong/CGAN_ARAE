...
class Corpus(object):
    def __init__(self, path, maxlen,clusterlist, vocab_size=11000, lowercase=False):
...
#         self.make_vocab()
        self.make_vocab_kong(clusterlist)

#         self.train = self.tokenize(self.train_path)
#         self.test = self.tokenize(self.test_path)
        self.train_list = self.tokenize_kong(clusterlist) # kong
        self.test_list = self.tokenize_kong(clusterlist) # kong
        
    def make_vocab(self):
        assert os.path.exists(self.train_path)
        # Add words to the dictionary
        with open(self.train_path, 'r', encoding='CP949') as f:
            for line in f:
                #if self.lowercase:
                    # -1 to get rid of \n character
    ...
    def make_vocab_kong(self, clusterlist):
    ...
    def tokenize_kong(self, clusterlist):
        """Tokenizes a text file."""
        dropped = 0
        linecount = 0
        lines = []
        for x_, y_ in clusterlist:
            indicelist=[]
            linecount += 1
            #if self.lowercase:
                #words = line[:-1].lower().strip().split(" ")
            #else:
            words = x_[:].strip().split(" ")
            if len(words) > self.maxlen:
                dropped += 1
                continue
            words = ['<sos>'] + words
            ...
            indicelist.append(y_)
            lines.append(indicelist)

        print("Number of sentences dropped from train_data: {} out of {} total".
...
def batchify_cluster(data, bsz, shuffle=False, gpu=False):
    if shuffle:
        random.shuffle(data)
    nbatch = len(data) // bsz
    batches = []
    ...
    for i in range(nbatch):
        # Pad batches to maximum sequence length in batch
        batch = np.asarray(data)[i*bsz:(i+1)*bsz,0].tolist()
        clusterbatch = np.asarray(data)[i*bsz:(i+1)*bsz,1].tolist()
#         print(batch)
        # subtract 1 from lengths b/c includes BOTH starts & end symbols
        lengths = [len(x)-1 for x in batch]
        # sort items by length (decreasing)
        batch, lengths = length_sort(batch, lengths)
        cluster = [x for x in clusterbatch]
        # source has no end symbol
        source = [x[:-1] for x in batch]
        # target has no start symbol
        target = [x[1:] for x in batch]
        ...
        source = torch.LongTensor(source)
        target = torch.LongTensor(target).view(-1)
        cluster = torch.LongTensor(np.array(cluster)).view(-1) # kong
        #source = torch.LongTensor(np.array(source))
        #target = torch.LongTensor(np.array(target)).view(-1)
        ...
        batches.append((source, target, lengths, cluster))
